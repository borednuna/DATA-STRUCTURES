...
void queueArray_init(QueueArray *queue, unsigned queueSize)
{
    queue->_element = (char**) malloc(sizeof(char*) * queueSize);
    queue->_front = -1;
    queue->_rear  = -1;
    queue->_size  = 0;
    queue->_capacity = queueSize;
    for (int i = 0; i < 100; i++)
    queue->_element[i] = (char*) malloc(sizeof(char)*(100+1));
}
...
void queueArray_push(QueueArray *queue, char* value)
{
...
char *queueArray_front(QueueArray *queue) 
{
    if (!queueArray_isEmpty(queue)) {
